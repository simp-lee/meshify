...
func ValidateRenderedSite(site SiteConfig, content []byte) error {
	...
	if strings.Contains(text, "listen 443 ssl http2") || strings.Contains(text, "listen [::]:443 ssl http2") {
		errs = append(errs, "lanpanel Nginx site must not use deprecated listen ... http2 syntax")
	}
...
func validateServerNameIsolation(errs *[]string, site SiteConfig, text string) {
	...
	for _, block := range blocks {
		if strings.Contains(block, "server_name _;") {
			*errs = append(*errs, "lanpanel Nginx site must not define pseudo catch-all server_name _")
		}
		if !strings.Contains(block, "default_server") {
			continue
		}
		if strings.Contains(block, "server_name "+site.ServerName+";") {
			*errs = append(*errs, "lanpanel named Nginx site must not be the default_server")
		}
		if strings.Contains(block, "proxy_pass ") || strings.Contains(block, "headscale_upstream") {
			*errs = append(*errs, "lanpanel default_server catch-all must not proxy to Headscale")
		}
		if !strings.Contains(block, `server_name "";`) {
			*errs = append(*errs, `lanpanel default_server catch-all must use empty server_name ""`)
		}
		...
		case hasHTTPDefault:
			...
			if !strings.Contains(block, "location /.well-known/acme-challenge/") || !strings.Contains(block, "root "+site.Webroot+";") {
				*errs = append(*errs, "lanpanel HTTP default_server catch-all must serve the ACME challenge webroot")
			}
			if !strings.Contains(block, "return 444;") {
				*errs = append(*errs, "lanpanel HTTP default_server catch-all must close unmatched requests with 444")
			}
		case hasHTTPSDefault:
			...
			if !strings.Contains(block, "ssl_certificate "+site.Fullchain+";") || !strings.Contains(block, "ssl_certificate_key "+site.PrivKey+";") {
				*errs = append(*errs, "lanpanel HTTPS default_server catch-all must use stable lanpanel certificate paths")
			}
			if !strings.Contains(block, "return 421;") {
				*errs = append(*errs, "lanpanel HTTPS default_server catch-all must reject unmatched SNI/Host with 421")
			}
		}
	}
	if !httpDefaultIPv4 || !httpDefaultIPv6 {
		*errs = append(*errs, "lanpanel Nginx site missing HTTP default_server catch-all")
	}
	if !httpsDefaultIPv4 || !httpsDefaultIPv6 {
		*errs = append(*errs, "lanpanel Nginx site missing HTTPS default_server catch-all")
	}
}

func validateHostSNIGuards(errs *[]string, site SiteConfig, text string) {
	hostMap := nginxBlockStartingWith(text, "map $http_host $lanpanel_host_header_valid")
	if hostMap == "" {
	...
	sniMap := nginxBlockStartingWith(text, "map $ssl_server_name $lanpanel_sni_valid")
	if sniMap == "" {
	...
	if block == "" {
		*errs = append(*errs, "lanpanel named HTTPS server block missing")
		return
	}
	...
	for _, guard := range []string{
		...
		if guardIndex < 0 {
			*errs = append(*errs, "lanpanel named HTTPS server block missing "+guard+" guard")
			continue
		}
		returnIndex := strings.Index(block[guardIndex:], "return 421;")
		if returnIndex < 0 {
			*errs = append(*errs, "lanpanel named HTTPS server block "+guard+" guard must return 421")
		}
		if proxyIndex >= 0 && guardIndex > proxyIndex {
			*errs = append(*errs, "lanpanel named HTTPS server block must reject Host/SNI mismatches before proxying")
		}
	}
}
